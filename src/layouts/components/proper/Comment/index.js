...
import { createComment, createReactionComment, deleteCommentChildren, deleteCommentParent, editComment, getComment } from '../../../../API/commentRequest';
...
import { reactDataEmoijs } from './datareact';
...
function Comment() {
    ...
    useEffect(() => {
        if (context.comments && searchPar.get('id')) {
            getDataComments('Comment', searchPar.get('id'), 1, "")
        }
        // eslint-disable-next-line
    }, [context.comments, searchPar.get('id')])
    const handleGetReplyDataOneTime = (id) => {
        if (allInfoOfComments.watchReply && !allInfoOfComments.convertReplyIDs?.includes(id)) {
            getDataComments('reply-Comment', searchPar.get('id'), 1, id)
    ...
    const handleCreateWithComment = async (type, courseID, replyID, value) => {
        try {
            ...
            if (data.success === 1) {
                if (type === 'reply-Comment') {
                    getDataComments(type, searchPar.get('id'), 1, replyID)
                    getDataComments('Comment', searchPar.get('id'), 1, "")
                    ...
                    getDataComments(type, searchPar.get('id'), 1, "")
    ...
    const handleCreateComment = () => {
        handleCreateWithComment('Comment', searchPar.get('id'), null, allInfoOfComments.valueComment)
    }
    const handleCreateReplyComment = (id) => {
        handleCreateWithComment('reply-Comment', searchPar.get('id'), id, allInfoOfComments.valueReplyComment)


    }
    const hanldeDeleteCommentParent = async (commenttable_id) => {
        try {
            dispatch(setDeleteMessage({ comments: allInfoOfComments.comments, commenttable_id, parentID: "", course_id: searchPar.get('id'), type: "delete" }))
    ...
    const hanldeDeleteCommentChildren = async (firstId, commenttable_id) => {
        try {
            dispatch(setDeleteMessage({ comments: allInfoOfComments.replyComments, commenttable_id, parentID: firstId, course_id: searchPar.get('id'), type: "delete-children" }))
            const { data } = await deleteCommentChildren(firstId, commenttable_id)
            if (data.success === 1) {
                getDataComments('reply-Comment', searchPar.get('id'), 1, firstId)
                getDataComments('Comment', searchPar.get('id'), 1, "")
            } else {
                toast.warn(data.message, {
                    position: "top-center",
    ...
    const handleEditComment = async (id, type) => {
        try {
            ...
            if (type === "reply-Comment") {
                dispatch(setEditMessage({ ...datas, commenttable_id: id, type: "edit-replyComment", course_id: searchPar.get('id'), comments: allInfoOfComments.replyComments }))
            } else {
                dispatch(setEditMessage({ ...datas, commenttable_id: id, type: "edit-Comment", course_id: searchPar.get('id'), comments: allInfoOfComments.comments }))
            }
            const { data } = await editComment(id, datas)
            if (data.success === 1) {
                getDataComments('reply-Comment', searchPar.get('id'), 1, allInfoOfComments.replyID)
    ...
    useEffect(() => {
        if (allInfoOfComments.editMessage) {
            socket.current.emit("update-comment", allInfoOfComments.editMessage)
        }
        // eslint-disable-next-line
    }, [allInfoOfComments.editMessage])
    useEffect(() => {
        if (searchPar.get('id')) {
            socket.current.on("update-respond-comment", datas => {
                dispatch(setRecievMessage(datas))
            })
    ...
    useEffect(() => {
        ...
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.commenttable_type === 'Comment') {
        ...
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.commenttable_type === 'reply-Comment') {
        ...
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.type === 'delete') {
            dispatch(setComments(allInfoOfComments.recieveMessage.newComments))
            dispatch(setRecievMessage(null))
        }
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.type === 'delete-children') {
            const newComments = allInfoOfComments.comments.map((cm) => {
        ...
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.type === 'edit-Comment') {
            dispatch(setComments(allInfoOfComments.recieveMessage.comments))
            dispatch(setRecievMessage(null))
        }
        if (allInfoOfComments.recieveMessage && allInfoOfComments.recieveMessage.course_id === searchPar.get('id') && allInfoOfComments.recieveMessage.type === 'edit-replyComment') {
                        ...
                        {allInfoOfComments.comments.length <= 0 ? <div className={cx('no_comments')}> <img src={NoCM} alt='nocomments' />
                            ...
                            allInfoOfComments.comments?.map((com) => <>
                                                    ...
                                                    <Tippy
                                                        ...
                                                        render={attrs => (
                                                            <div >
                                                                <div style={{
                                                                    ...
                                                                    {reactDataEmoijs.map((val) =>
                                                                        <div key={val.id} className={cx('parent-emj')} style={{ width: "48px" }} onClick={() => handleReactionComments(com.commenttable_id, val.id, searchPar.get('id'))}>
                                                                            <span>
                                                                                <div>
                                                                                    <div style={{
                                                                                        padding: "5px",
                                                        ...
                                                        {com.reactionText ?
                                                            <button className={cx('iconWrapper')} onMouseOver={() => {
                                                                ...
                                                                onClick={() => handleReactionComments(com.commenttable_id, allInfoOfComments.textReaction[com.reactionText], searchPar.get('id'))}
                                                            >
                                                            ...
                                                            <button className={cx('iconWrapper')} onMouseOver={() => {
                                                                ...
                                                                onClick={() => handleReactionComments(com.commenttable_id, "like", searchPar.get('id'))}
                                                            >
                                ...
                                {com.comment_count !== 0 && allInfoOfComments.watchReply && allInfoOfComments.replyComments.map((rep) =>
                                    allInfoOfComments.replyIDs.includes(com.commenttable_id) && rep.reply_id === com.commenttable_id && <div key={rep.commenttable_id} className={cx('reply')}>
                                        <div className={cx('detailComment', 'relyComment')} style={{
                                                        ...
                                                        <div className={cx('creatAt')}>
                                                            <div className={cx('createdAtLeft')} >
                                                                <Tippy
                                                                    ...
                                                                    render={attrs => (
                                                                        <div >
                                                                            <div style={{
                                                                                ...
                                                                                {reactDataEmoijs.map((val) =>
                                                                                    <div key={val.id} className={cx('parent-emj')} style={{ width: "48px" }} onClick={() => handleReactionComments(rep.commenttable_id, val.id, searchPar.get('id'))}>
                                                                                        <span>
                                                                                            <div>
                                                                                                <div style={{
                                                                                                    padding: "5px",
                                                                    ...
                                                                    {rep.reactionText ?
                                                                        <button className={cx('iconWrapper')} onMouseOver={() => {
                                                                            ...
                                                                            onClick={() => handleReactionComments(rep.commenttable_id, allInfoOfComments.textReaction[rep.reactionText], searchPar.get('id'))}
                                                                        >
                                                                        ...
                                                                        <button className={cx('iconWrapper')} onMouseOver={() => {
                                                                            ...
                                                                            onClick={() => handleReactionComments(rep.commenttable_id, "like", searchPar.get('id'))}
                                                                        >
                                                                            <span className={cx('likeComment')}>Thích</span>
                                                                        </button>}
                                                                </Tippy>
                                                                ...
                                                                <span onClick={() => {
                                                                    ...
                                                                    dispatch(setReplyCommentID(rep.commenttable_id))
                                                                    dispatch(setReplyCommentIDs(rep.commenttable_id))
                                                                }} className={cx('repleComment')}>Trả lời</span>
                                                            ...
                                                            <div className={cx('createdAtRight')}>
                                                                ...
                                                                <span className={cx('moreBntWrapper')}>
                                                                    ...
                                                                    <Tippy
                                                                        interactive
                                                                        visible={allInfoOfComments.showOption && rep.commenttable_id === allInfoOfComments.showOptionID}
                                                                        onClickOutside={() => {
                                                                            dispatch({ type: SHOWOPTION, payload: false })
                                                                        ...
                                                                        <button onClick={() => {
                                                                            dispatch({ type: SHOWOPTION, payload: true })
                                                                            dispatch(setShowOptionID(rep.commenttable_id))
                                                                        }} className={cx('moreBnt')}><FontAwesomeIcon icon={faEllipsis} /></button>
                                            ...
                                            {allInfoOfComments.activeReply && allInfoOfComments.replyCommentIDs?.includes(rep.commenttable_id) &&
                                                <div className={cx('reply_BoxComment')}>
                                                    <div className={cx('replyContent')}>
                                                        <div className={cx('avatarcomment')}>
                                                            <div className={cx('avatarwrapperComment')}>
                                                    ...
                                                    <div className={cx('actionwrapper_reply')}>
                                                        <Button normal onClick={() => {
                                                            dispatch(deleteReplyCommentIDs(rep.commenttable_id))
                                                            handleSetValueReplyComment('')
                                                            if (allInfoOfComments.activeEditComment) {
